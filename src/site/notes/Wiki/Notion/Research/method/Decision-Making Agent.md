...
**“长时-程复杂问题 (Long-Horizon Complex Problems)”** 不仅仅是DMA领域的一个重要方向，它更是当前**定义和衡量前沿决策智能体能力的核心标尺**。
...
1. agent上限由三个主要因素组成：environment ($R$), priority ($\theta^*$), algorithm ($RL$)
2. 其中 priority ($\theta^*$), algorithm ($RL$) 都是人类比较能优化的地方，因此agent上限主要由 environment ($R$) 所限制 (因此设计的 priority ($\theta^*$), algorithm ($RL$) 最好是能让agent自己能够从环境中直接学习)
3. 环境可以视作一种**图状结构**。大部分的人造环境都是**有限图**，即存在某几个点是这个环境的上限，在这个环境中的agent上限被这些点约束。但是世界是**无限树**，一个无限大的图，因此里面的agent上限是无限的 (因此agent要更进一步必须在更复杂乃至真实时间进行迭代)